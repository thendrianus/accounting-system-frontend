...
@Component({
  selector: 'purchase_order',
  styleUrls: ['./purchase_order.scss'],
  templateUrl: './purchase_order.html',
})
export class Purchase_order {

  purchase_ordersDetail: any = [];

  formPurchase_order;
  formPurchase_orderDetail;
  formDelivery;
  formTerm;
  ...
  _formPurchase_orderDetail = { purchase_order_detail_id: '', purchase_order_id: '', inventory_id: '', inventory_code: '', name: '', warehouse_id: '', ordered: 1, orderedeqv: 1, inventory_hpp: 0, delivered: 0, deliveredeqv: 0, price: 0, landed_cost: 0, discount_persent: 0, discount_amount: 0, uom_order: '', uom_label: '', row_order: 1, row_label: '', isdelivery: 0, tax_id: '', tax: 0, delivery_schedule: new Date(), delivery_note: '-', inventory_label: '', description: '-', create_by: '-', update_by: '-', create_datetime: new Date(), update_datetime: new Date(), is_use: 1, is_active: 1, oriprice: 0, uom1: '', uom2: '', uom3: '', uom2equal: 1, uom3equal: 1, min_price: 0, selling_price: 0, selling_price2: 0, selling_price3: 0, selling_dsc_amount: 0, selling_dsc_amount2: 0, selling_dsc_amount3: 0, stock: 0, purchase_receive_qty: 0 }
  ...
  constructor(
    ...
    this.formPurchase_order = this.formBuilder.group({ //sssss
      purchase_order_id: '',
      purchase_order_detail: [],
      purchase_order_code: ['', [Validators.minLength(0), Validators.maxLength(20)]],
      currency_id: '',
    ...
    this.formDelivery = this.formBuilder.group({ //sssss
      ...
      delivery_note: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(20)]],
    ...
    this.formPurchase_orderDetail = this.formBuilder.group({ //sssss
      ...
      discount_persent: [0],
      discount_amount: [0],
      uom_order: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(20)]],
      uom_label: '',
      row_order: 1,
      row_label: '',
      isdelivery: 0,
  ...
  disabledSpecialFormField() {
    ...
    this.disableInput.purchase_order_code = true;
  }

  formFieldDisabledEnabled(io) {
    if (io == 0) {
      for (let e of this.formPurchase_orderField) {
  ...
  getGen() {
    this.httpService.getTranslate('34').subscribe(
      value => {
        if (value.hasOwnProperty("app_component_id")) {
          ...
          this.getPurchase_ordercategory();
          this.getpurchasesman();
          this.getPurchase_ordersStatus();
        } else {
  ...
  setpurchase_order(data) {
    if (data) {
      ...
      this.formPurchase_order.patchValue(this.copying(data));
    } else {
      this.formPurchase_order.reset(this._formPurchase_order)
    }
  ...
  @ViewChild('childModal2') public childModal2: ModalDirective; // purchase_order detail form
  @ViewChild('childModal3') public childModal3: ModalDirective; //purchase_order delivery form
  @ViewChild('childModal4') public childModal4: ModalDirective; //purchase_order search
  ...
  modalShow(item) {
    ...
    if (item == '-') {
      ...
      this.modal2Out(this.purchase_ordersDetail[item]);
  ...
  modalOut() {
    ...
    if (this.ifModal2show == -1) {
      this.purchase_ordersDetail.push(this.copying(this.formPurchase_orderDetail.getRawValue()));
    } else {
      this.purchase_ordersDetail[this.ifModal2show] = this.copying(this.formPurchase_orderDetail.getRawValue());
    }
    this.formPurchase_orderDetail.reset(this._formPurchase_orderDetail);
    this.modalHide();
    this.count();

    // for (let item in this.purchase_ordersDetail) {
    //   if (item) {
  ...
  mySettings() {
    return {
      ...
      columns: this.httpService.generateng2columns({
        ...
        buying_price: {
          title: this.gen.th_selling_priceTbl2,
          type: 'string',
          show: 1
        },
        stock: {
  ...
  modal2Out(event) {
    console.log(event)
    for (let index = 0; index < this.purchase_ordersDetail.length; index++) {

      if (this.purchase_ordersDetail[index].inventory_id == event.inventory_id) {

        if (this.purchase_ordersDetail[index].is_active == 1) {
    ...
    if (this.ifModal2show == -1) {
      ...
      this.formPurchase_orderDetail.patchValue(this.copying(this.purchase_ordersDetail[this.ifModal2show]))
    }

    if (this.formPurchase_orderDetail.value.inventory_id != event.inventory_id) {
      this.formPurchase_orderDetail.patchValue({
        uom_order: 1,
      ...
      if (!this.formPurchase_orderDetail.value.inventory_label) {
        this.formPurchase_orderDetail.patchValue({ inventory_label: event.name });
      }
  ...
  warehouseChange() {
    this._formPurchase_orderDetail.warehouse_id = this.formPurchase_order.value.warehouse_id;
  }

  taxChange() {
    this._formPurchase_orderDetail.tax_id = this.formPurchase_order.value.tax_id;
  }
    ...
    this.httpService.http_api_post('apps/warehouse/select', { is_use: '1' }).subscribe((value) => {
      ...
      if (value.success) {
        ...
        if (value.data.warehouse.length > 0) {
          this._formPurchase_order.warehouse_id = value.data.warehouse[0].warehouse_id;
          this.formPurchase_order.patchValue({ warehouse_id: value.data.warehouse[0].warehouse_id });
          this._formPurchase_orderDetail.warehouse_id = value.data.warehouse[0].warehouse_id;
        }
      }
    ...
    this.httpService.http_api_post('apps/tax/select', { is_use: '1' }).subscribe((value) => {
      ...
      if (value.success) {
        ...
        if (value.data.tax.length > 0) {
          this._formPurchase_order.tax_id = value.data.tax[0].tax_id;
          this.formPurchase_order.patchValue({ tax_id: value.data.tax[0].tax_id });
          this._formPurchase_orderDetail.tax_id = value.data.tax[0].tax_id;
        }
      }
    ...
    this.httpService.http_api_post('company/businesspartner/select', { is_use: '1', businesspartner_category_id: 3 }).subscribe((value) => {
      ...
      if (value.success) {
        ...
        if(this.businesspartnerData[0]){
          this.formPurchase_order.patchValue({
            businesspartner_id: this.businesspartnerData[0].businesspartner_id
          })
          this._formPurchase_order.businesspartner_id= this.businesspartnerData[0].businesspartner_id
        }
      }
    ...
    this.httpService.http_api_post('apps/branch/s', {}).subscribe((value) => {
      ...
      if (value.success) {
        ...
        if (value.data.branch.length > 0) {
          this._formPurchase_order.branch_id = value.data.branch[0].branch_id;
          this.formPurchase_order.patchValue({ branch_id: value.data.branch[0].branch_id });
        }
      }
    ...
    this.httpService.http_api_get('transaction/purchase/category/').subscribe((value) => {
      ...
      if (value.success) {
        ...
        if (value.data.purchase_category.length > 0) {
          this._formPurchase_order.purchase_category_id = value.data.purchase_category[0].purchase_category_id;
        }
      }
    ...
    this.httpService.http_api_get('transaction/purchase/payment/').subscribe((value) => {
      ...
      if (value.success) {
        ...
        if (value.data.transaction_payment.length > 0) {
          this._formPurchase_order.transaction_payment_id = value.data.transaction_payment[0].transaction_payment_id;
        }
      }
    ...
    this.httpService.http_api_put('hrd/employee/select', { action: 'salesman' }).subscribe((value) => {
      ...
      if (value.success) {
        ...
        if (value.data.employee.length > 0) {
          this._formPurchase_order.purchasesman_id = value.data.employee[0].employee_id;
          this.formPurchase_order.patchValue({ purchasesman_id: value.data.employee[0].employee_id });
        }
      }
  ...
  purchase_ordersDetailDelete(item) {
  ...
  modal4Out(item) {
    ...
    this.setpurchase_order(item.data);

    if (this.formPurchase_order.value.oripurchase_status_id != 5) {
      this.formFieldDisabledEnabled(1);
    } else {
      this.formFieldDisabledEnabled(0);
    }

    this.getPurchase_ordersDetail();
    this.modal4Hide();
  }
  ...
  getPurchase_orderList() {
    this.httpService.http_api_post('transaction/purchase_orders/select', { is_use: 0, action: 0 }).subscribe((value) => {

      if (value.success) {
        this.source2.load(value.data.purchase_orders);
      }
  ...
  getPurchase_ordersDetail() {
    this.httpService.http_api_post('transaction/purchase_orders/detail', { purchase_order_id: this.formPurchase_order.value.purchase_order_id }).subscribe((value) => {

      if (value.success) {
        this.purchase_ordersDetail = value.data.purchase_order_detail;
        this.count();
      }
  ...
  setCurrencyData(event) {
    this._formPurchase_order.currency_id = event.currency_id;
    this._formPurchase_order.rate = event.rate;
    this.formPurchase_order.patchValue({
  ...
  purchase_orderParse: any;
  purchase_orderSubmit(status) {

    this.count();
    this.purchase_orderParse = this.copying(this.formPurchase_order.getRawValue());
    if (status != 0) {
      this.purchase_orderParse.purchase_status_id = status;
    }
    this.purchase_orderParse.purchase_order_detail = this.copying(this.purchase_ordersDetail);
    this.purchase_orderParse.update_by = this.httpService.currentUser.employee_id;
    this.purchase_orderParse.update_datetime = new Date();

    this.purchase_orderParse.orisub_total = this.purchase_orderParse.sub_total;
    this.purchase_orderParse.oritax = this.purchase_orderParse.tax;
    this.purchase_orderParse.origrand_total = this.purchase_orderParse.grand_total;
    this.purchase_orderParse.oridownpayment = this.purchase_orderParse.downpayment;
    this.purchase_orderParse.orilanded_cost = this.purchase_orderParse.landed_cost;
    this.purchase_orderParse.oripayable = this.purchase_orderParse.payable;

    if (this.purchase_orderParse.purchase_order_id == '') {
      this.purchase_orderParse.create_by = this.httpService.currentUser.employee_id;
      this.purchase_orderParse.create_datetime = new Date();

      this.httpService.http_api_post('transaction/purchase_orders', this.purchase_orderParse)
        .subscribe((value) => {

          if (value.success) {
            this.notif.success = { title: 'Success', content: value.label, setting: this.httpService.success, change: Math.random().toString() };
            this.setpurchase_order(this.purchase_orderParse);

            this.formPurchase_order.patchValue({
              purchase_order_code: value.data.purchase_order_code,
              purchase_order_id: value.data.purchase_order_id,
              oripurchase_status_id: this.formPurchase_order.value.purchase_status_id,
            })
  ...
  saveUpdate() {
    ...
    this.httpService.http_api_put('transaction/purchase_orders', this.purchase_orderParse)
      .subscribe((value) => {
        if (value.success) {
          this.notif.success = { title: 'Success', content: value.label, setting: this.httpService.success, change: Math.random().toString() };
          if (this.formPurchase_order.value.is_active == 0) {
            this.clearAll();
          } else {
            if (value.data.purchase_order_code) {
              this.purchase_orderParse.purchase_order_code = value.data.purchase_order_code;
            }
            this.setpurchase_order(this.purchase_orderParse);
            this.formPurchase_order.patchValue({ oripurchase_status_id: this.formPurchase_order.value.purchase_status_id });
            if (this.formPurchase_order.value.oripurchase_status_id != 5) {
              this.formFieldDisabledEnabled(1);
            } else {
              this.formFieldDisabledEnabled(0);
  ...
  purchase_orderDelete() {
    if (confirm("Are you sure to delete this data?")) {
      this.formPurchase_order.patchValue({ is_active: 0 });
      this.purchase_orderSubmit(0);
    }
  }
  ...
  purchase_ordersDetailChange(value) {
    this.count();
  }

  clearAll() {
    this.setpurchase_order(false);

    this.formFieldDisabledEnabled(1);

    this.purchase_ordersDetail = JSON.parse("[]");
    this.formPurchase_orderDetail.reset(this._formPurchase_orderDetail);
    this.purchase_orderDetailOrderedCount = 0;
    this.purchase_orderDetailDeliveredCount = 0;
    this.purchase_orderDetailDiscountCount = 0;
    this.purchase_orderDetailPriceCount = 0;
    this.purchase_orderDetailLength = 0;

    this.disableInput.reference_code = false;

  }

  purchase_orderDetailOrderedCount = 0;
  purchase_orderDetailDeliveredCount = 0;
  purchase_orderDetailDiscountCount = 0;
  purchase_orderDetailPriceCount = 0;
  //purchase_orderdetailcount = data length
  //total sum = purchase_order sub_total
  purchase_orderDetailLength = 0;

  count() {

    if (this.purchase_ordersDetail.length > 0) {
      this.purchase_orderDetailOrderedCount = 0;
      this.purchase_orderDetailDeliveredCount = 0;
      this.purchase_orderDetailDiscountCount = 0;
      this.purchase_orderDetailPriceCount = 0;
      ...
      for (let e of this.purchase_ordersDetail) {

        if (e.is_active == 1) {
          e.discount_amount = (e.price * e.ordered) * e.discount_persent / 100;

          this.purchase_orderDetailOrderedCount += e.ordered;
          this.purchase_orderDetailDeliveredCount += e.delivered;
          this.purchase_orderDetailDiscountCount += e.discount_amount;
          this.purchase_orderDetailLength += 1;
          this.purchase_orderDetailPriceCount += e.price;

          var sub_totalParse = (e.price * e.ordered) - e.discount_amount + e.landed_cost;


          this.formPurchase_order.patchValue({
            detail_landed_cost: this.formPurchase_order.value.detail_landed_cost + e.landed_cost,
            sub_total: this.formPurchase_order.value.sub_total + sub_totalParse
          })
          ...
          for (let f of this.taxData) {
            ...
            if (e.tax_id == f.tax_id) {
              ...
              this.formPurchase_order.patchValue({
                tax: this.formPurchase_order.value.tax += e.tax
              })
            }
      ...
      var grand_total = this.formPurchase_order.value.sub_total + this.formPurchase_order.value.landed_cost + this.formPurchase_order.value.tax - this.formPurchase_order.value.discount_amount;

      this.formPurchase_order.patchValue({
        grand_total: grand_total,
        payable: grand_total - this.formPurchase_order.value.downpayment,
        downpayment_persent: this.formPurchase_order.value.downpayment / grand_total * 100
      })
  ...
  purchase_orderDiscountCountAmount(event) {
    if (this.purchase_ordersDetail.length > 0) {
      this.formPurchase_order.patchValue({ discount_amount: (this.formPurchase_order.value.sub_total + this.formPurchase_order.value.landed_cost) * this.formPurchase_order.value.discount_persent / 100 });
      this.count();
    }
  }

  purchase_orderDiscountCountPersent(event) {
    if (this.purchase_ordersDetail.length > 0) {

      this.formPurchase_order.patchValue({ discount_persent: this.formPurchase_order.value.discount_amount / (this.formPurchase_order.value.sub_total + this.formPurchase_order.value.landed_cost) * 100 });
      this.count();
    }
  }

  purchase_orderDownpaymentCountAmount(event) {
    if (this.purchase_ordersDetail.length > 0) {
      this.formPurchase_order.patchValue({ downpayment: this.formPurchase_order.value.grand_total * this.formPurchase_order.value.downpayment_persent / 100 });
      this.count();
    }
  }

  purchase_orderDownpaymentCountPersent(event) {
    if (this.purchase_ordersDetail.length > 0) {
      this.formPurchase_order.patchValue({ downpayment_persent: this.formPurchase_order.value.downpayment / this.formPurchase_order.value.grand_total * 100 });
      this.count();
    }
  }

  purchase_orderDetailUomOrderChange() {

    if (this.formPurchase_orderDetail.value.uom_order == 1) {
      this.formPurchase_orderDetail.patchValue({
        orderedeqv: this.formPurchase_orderDetail.value.ordered,
        deliveredeqv: this.formPurchase_orderDetail.value.delivered,
        uom_label: this.formPurchase_orderDetail.value.uom1,
        price: this.formPurchase_orderDetail.value.buying_price,
        min_price: this.formPurchase_orderDetail.value.selling_dsc_amount,
      })
    } else if (this.formPurchase_orderDetail.value.uom_order == 2) {
      this.formPurchase_orderDetail.patchValue({
        orderedeqv: this.formPurchase_orderDetail.value.uom2equal * this.formPurchase_orderDetail.value.ordered,
        deliveredeqv: this.formPurchase_orderDetail.value.uom2equal * this.formPurchase_orderDetail.value.delivered,
        uom_label: this.formPurchase_orderDetail.value.uom2,
        price: this.formPurchase_orderDetail.value.selling_price2,
        min_price: this.formPurchase_orderDetail.value.selling_dsc_amount,
      })
    } else if (this.formPurchase_orderDetail.value.uom_order == 3) {
      this.formPurchase_orderDetail.patchValue({
        orderedeqv: this.formPurchase_orderDetail.value.uom3equal * this.formPurchase_orderDetail.value.ordered,
        deliveredeqv: this.formPurchase_orderDetail.value.uom3equal * this.formPurchase_orderDetail.value.delivered,
        uom_label: this.formPurchase_orderDetail.value.uom3,
        price: this.formPurchase_orderDetail.value.selling_price3,
        min_price: this.formPurchase_orderDetail.value.selling_dsc_amount,
      })
  ...
  modalImportOut(item) {
    ...
    this.formPurchase_order.patchValue(data)

    if (item.purchase_quotation_id) {
      this.formPurchase_order.patchValue({
        reference_label: 'Purchase Quotation',
  ...
  setImportDetail(item) {
    ...
    for (var i in item) {
      ...
      this.purchase_ordersDetail.push(this.copying({
        ...this._formPurchase_orderDetail,
        ...data
      }));
  ...
  purchaseItemPriceDiscountCountAmount(event) {
    ...
    if (this.formPurchase_orderDetail.value.price != 0) {
      this.formPurchase_orderDetail.patchValue({ discount_amount: this.formPurchase_orderDetail.value.price * this.formPurchase_orderDetail.value.discount_persent / 100 });
    } else {
      this.formPurchase_orderDetail.patchValue({ discount_persent: 0 });
    }
  }
  ...
  purchaseItemPriceDiscountCountPersent(event) {
    ...
    if (this.formPurchase_orderDetail.value.price != 0) {
      this.formPurchase_orderDetail.patchValue({ discount_persent: this.formPurchase_orderDetail.value.discount_amount / this.formPurchase_orderDetail.value.price * 100 });
    } else {
      this.formPurchase_orderDetail.patchValue({ discount_amount: 0 });
    }
  }
  ...
  getBusinesspartneraddress() {
    ...
    console.log(this.formPurchase_order.value.businesspartner_id);
    if (this.formPurchase_order.value.businesspartner_id != '') {
      this.httpService.http_api_post('company/businesspartner/addresslist', { businesspartner_id: this.formPurchase_order.value.businesspartner_id })
        .subscribe((value) => {
  ...
  selectedCustomer(event) {
    ...
    this._formPurchase_order.discount_date = event.discount_date;
    this._formPurchase_order.due_date = event.due_date;
    this._formPurchase_order.early_discount = event.early_discount;
    this._formPurchase_order.late_charge = event.late_charge;
  }
